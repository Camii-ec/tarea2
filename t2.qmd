...
Ahora para la Regresión Ridge, el ajuste es bastante similar, pero la principal variación, ocurre en que el $\alpha$ se fija en $0$, resultando:
...
Con lo anterior, es posible determinar los valores de los predictores de la regresión Ridge, son:

```{r echo = FALSE}

tabla_datos2 <- as.matrix(ridge_2.0)

colnames(tabla_datos2) <- c("Predictores Ridge")

knitr::kable(tabla_datos2, digits = 4, align = "c") %>% 
  kable_styling( position="center")
...
tabla_datos5 <- cbind(tabla_datos1,tabla_datos2,tabla_datos3,tabla_datos4)

colnames(tabla_datos5) <- c("Lasso","Ridge","Elastic-Net","Lasso Adaptativo")
...
i) Compare el MSE promedio obtenido en los set de datos de testeo considerando (a) mínimos cuadrados, (b) regresión Ridge, (c) LASSO, (d) Elastic-Net y (e) LASSO adaptativo.
...
Una vez desarmada la base se tiene:




ii)  Visualize los resultados obtenidos para comparar los modelos. Muestre los resultados solamente para el mejor par´ametro de calibración elegido

iii) ¿Qué métodos generan el mejor error de predicción? ¿Por qué estos métodos funcionan bien? ¿Los métodos eligen el mismo subconjunto de variables? Explique y amplíe sus respuestas.
